...
bool check_parentheses(int p, int q, bool *success, int *position) { // DOWN
   ...
   * 我们约定规则如下:
   * 0,
   * tokens的扫描从左往右，一次扫描应当进行到底，遇到tokens组合错误立即设置错误位置，设置success
   * 1, 任何一个表达式应当以'('开始，以')'结尾
   * 2, 任意多的'('就应当有任意多的')'与之相匹配
   * 3, 任何时候'('的数量都应当大于')'的数量
   */
  bool is_parentheses =
      tokens[p].type == LBK; // 当违反规则1时，设置错误信息，以便后续返回
  int index = p,
      level =
          -1; // 默认从-1开始，表示此时没有匹配到任何一个前括号；使用level对前括号进行计数
...
word_t expr(char *e, bool *success) {
  ...
  for (size_t i = 0; i < nr_token; i++) {
    if (tokens[i].type == MUL &&
        (i == 0 || // i == 0是指第一个就要解引用的情况
         tokens[i - 1].type ==
             LBK)) { // 逻辑运算符短路求值，||运算符前面的满足时，后面的不再进行判断，否则出现错误
      tokens[i].type = DEREF;
    } else if (tokens[i].type == SUB && (i == 0 || tokens[i - 1].type == LBK ||
                                         tokens[i - 1].type == SUB)) {
      tokens[i].type = NEG;
    }
  }
...
word_t eval(int p, int q, bool *success, int *position) {
  ...
  if (p > q) { // Bad expression.
    ...
    *position = p;
    ...
    case NUM:
      sscanf(tokens[p].str, FMT_PRId, &buffer);
      break;
    case HEX:
      sscanf(tokens[p].str, FMT_PRIx, &buffer);
      break;
    case REG:
    ...
    return buffer;
      ...
      1 ==
      q - p) { // 【双token
               // 特殊运算符处理】任何时候当表达式已经被分割成两个连续的tokens时，总是意味着它们有特殊的含义
    word_t buffer = 0;
    switch (tokens[p].type) {
    case DEREF:
      // Log("DEREF");
      buffer = *((word_t *)guest_to_host(
          eval(p + 1, q, success, position))); // FIXME:传入奇怪地址时可能报错。
      break;
    case NEG:
      buffer = -eval(p + 1, q, success,
                     position); // FIXME:不能在()前面直接使用-，否则会无法匹配
      break;
    default:
      buffer = -1;
    ...
    return buffer;
  } else if (check_parentheses(p, q, success, position) ==
             true) { // 【括号处理】
    ...
    for (size_t i = p; i < q; i++) // 我们要求运算的符号都是右结合的
    {
      if (tokens[i].type == '(') {
        ...
        level--;
      } else if (-1 == level && prio(tokens[i].type) >
                                    0) { // 要找的主运算符不在某一个括号层次中
        if (-1 == op ||
            prio(tokens[i].type) <=
                prio(
                    tokens[op]
                        .type)) { // 由于||是短路求值的运算符，所以当前面的op ==
                                  // -1的时候，后面的就不会进行判断，避免出现数组下标越界的错误。
          op = i; // 记录下最高优先级的运算符
        }
      }
    }
    ...
    case TK_NEQ:
      return val1 != val2;
    default:
      Log("Unknown op type.Check here!");
      *success = false;
      *position = op;
      return 0;
    }
