...
WP *new_wp(char *condation, bool *success) { // DONE
  if (NULL == condation || NULL == success) {
    Log("Too few args. Should be: new_wp(const char* condition, bool "
        "*success).");
    return NULL;
  }
...
void watchpoint_display() {
  ...
  WP *cur = head;
  while (cur) {
    printf("\e" ANSI_FG_CYAN "%d\e" ANSI_NONE "\t\e" ANSI_FG_GREEN
           "%s\t" FMT_PRId "\t0x" FMT_PRIx "\e" ANSI_NONE "\n",
...
bool check_watchpoint() {
  ...
  while (cur) {
    ...
    if (value != cur->cur_value) {
      printf("\e" ANSI_FG_CYAN "(NO.%d):\t\e" ANSI_FG_YELLOW
             "Cond:\e" ANSI_FG_GREEN "%s\t\e" ANSI_FG_YELLOW
             "Cur_DEX:\e" ANSI_FG_GREEN "0d" FMT_PRId "\t\e" ANSI_FG_YELLOW
             "Cur_HEX:\e" ANSI_FG_GREEN "0x" FMT_PRIx "\t\e" ANSI_FG_YELLOW
             "New_DEX:\e" ANSI_FG_GREEN "0d" FMT_PRId "\t\e" ANSI_FG_YELLOW
             "New_HEX:\e" ANSI_FG_GREEN "0x" FMT_PRIx "\e" ANSI_NONE "\n",
             cur->NO, cur->condation, cur->cur_value, cur->cur_value, value,
             value);
      watch = true;
    }
    cur->cur_value = value;
